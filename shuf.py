...
def main():
    ...
    print("start timeit...")
    tt = timeit.timeit(
        "test3()", setup="from __main__ import test3", number=DEFAULT_NUMBER
    )
    print("  duration np.random.shuffle:", tt)
    tt = timeit.timeit(
        "test2()", setup="from __main__ import test2", number=DEFAULT_NUMBER
    )
    print("  duration random.shuffle:", tt)
    tt = timeit.timeit(
        "test1()", setup="from __main__ import test1", number=DEFAULT_NUMBER
    )

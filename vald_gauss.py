...
class ValidateGuassian:
    ...
    def fill_array(self):
        """fill array"""
        self.data_arr = np.random.normal(
            self.target_mean[0], self.target_stdev[0], self.target_array_size
        )
    ...
    def validate_array(self, arr):
        ...
        # print('mean: {:.3f}\nstdev: {:.3f}\n'.format(mean, stdev))
        if (
            abs(self.target_mean[0] - mean) < self.target_mean[1]
    ...
    def printOut(self):
        ...
        print(
            "mean: {:8.3f}, stdev: {:6.3f}".format(self.result_mean, self.result_stdev),
            end=" ",
        )

...
class DuplicateCodeRemover:
    ...
    def find_duplicate_blocks(self, lines, block_size=3):
        block_map = defaultdict(list)
        for i in range(len(lines) - block_size + 1):
            block = tuple(lines[i:i + block_size])
            block_map[block].append(i)
        duplicates = {k: v for k, v in block_map.items() if len(v) > 1}
        return duplicates
    ...
    def scan_and_remove_duplicates(self):
        ...
        for file in files:
            ...
            block_duplicates = self.find_duplicate_blocks(lines, block_size=3)
            
            i = 0
            while i < len(lines):
                block = tuple(lines[i:i + 3])
                if block in block_duplicates and block_duplicates[block]:
                    duplicates[block] += 1
                    i += 3  # Skip the duplicate block
                else:
            ...
            summary += f"Duplicate blocks removed: {len(block_duplicates)}\n"
            summary += f"Single-line duplicates logged: {sum(duplicates.values())}\n"

...
def bsinPoints(pb, pe):
    ...
    k1 = pb + (pe - pb) * (1.0 / 12.0) - y_alfa
    k2 = pb + (pe - pb) * (2.0 / 12.0) - y_beta
    k3 = pb + (pe - pb) * (4.0 / 12.0) - y_beta
    k4 = pb + (pe - pb) * (5.0 / 12.0) - y_alfa
    k5 = pb + (pe - pb) * (7.0 / 12.0) + y_alfa
    k6 = pb + (pe - pb) * (8.0 / 12.0) + y_beta
    k7 = pb + (pe - pb) * (10.0 / 12.0) + y_beta
    k8 = pb + (pe - pb) * (11.0 / 12.0) + y_alfa
...
def bcosPoints(pb, pe):
    ...
    p3 = pb + (pe - pb) * 0.75
    k1 = pb + (pe - pb) * (1.0 / 12.0) - y_beta
    k2 = pb + (pe - pb) * (2.0 / 12.0) - y_alfa
    k3 = pb + (pe - pb) * (4.0 / 12.0) + y_alfa
    k4 = pb + (pe - pb) * (5.0 / 12.0) + y_beta
    k5 = pb + (pe - pb) * (7.0 / 12.0) + y_beta
    k6 = pb + (pe - pb) * (8.0 / 12.0) + y_alfa
    k7 = pb + (pe - pb) * (10.0 / 12.0) - y_alfa
    k8 = pb + (pe - pb) * (11.0 / 12.0) - y_beta
    return p0, k1, k2, p1, k3, k4, p2, k5, k6, p3, k7, k8, p4
...
if __name__ == "__main__":
    ...
    for i in (0, 3, 6, 9):  # draw all 4 function segments
        page.drawBezier(pnt[i], pnt[i + 1], pnt[i + 2], pnt[i + 3], color=red, width=w)
    ...
    for i in (0, 3, 6, 9):  # draw all 4 function segments
        page.drawBezier(pnt[i], pnt[i + 1], pnt[i + 2], pnt[i + 3], color=blue, width=w)

...
class Codeview(ttk.Frame):
    def __init__(self, parent):
        ...
        self.notebook.add(self.f9, text="auto_pep_8")
        pep8 = AutoPEP8Formatter(self.f9)
    def storepath(self):
...
class SideButtonframe:
    ...
    def execute_method(self):
        ...
        if selected_method == "Open":
            ...
            self.indent(self.textwidget)
        elif selected_method == "Dedent":
            self.dedent(self.textwidget)
        elif selected_method == "adjust indent":
            self.indent_text(self.textwidget)
        elif selected_method == "adj dedent":
            self.dedent_text(self.textwidget)
    ...
    def indent2(self,textwidget, event=None):
    ...
    def auto_indent(self,textwidget, event=None):
        self.textwidget = event.widget

        # get leading whitespace from current line

    def apply_indent_dedent(textwidget):
    ...
    def indent_text(self,textwidget):
        text.tag_configure("indent", lmargin1=30)

    def dedent_text(self,textwidget):
        text.tag_configure("indent", lmargin1=0)

    def change_indent_spaces(self,textwidget):
    ...
    def apply_indent_dedent(self,textwidget):
        sel_start, sel_end = text.tag_ranges(tk.SEL)
        if sel_start and sel_end:
            text.tag_add("indent", sel_start, sel_end)
...
# The following code would be in your main application file

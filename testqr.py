...
def get_qrcode(text="The quick brown fox jumps over the lazy dog", showimage=True):
    ...
    payload = {"data": text, "size": "256x256", "ecc": "M"}
...
def main():
    ...
    parser.add_argument(
        "--isbn", nargs="?", help="ISBN need hypenated, eg: 978-986-998-168-2"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", default=True, help="show resp results"
    )
    parser.add_argument(
        "-s", "--showimage", action="store_true", help="show generated image"
    )

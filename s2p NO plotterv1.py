...
if filename:
    ...
    # Plot magnitude of S21 for example
    plt.figure()
    plt.plot(frequencies, [abs(s) for s in s21], label="|S21|")
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("Magnitude")
    plt.title("S21 Magnitude vs. Frequency")
    ax.scatter(frequencies, s11, s22, s=sizes, c=colors, vmin=0, vmax=100)
    ax.set(xlim=(-80, 80), xticks=np.arange(1, 8), ylim=(0, 8), yticks=np.arange(1, 8))
    plt.legend()
    plt.grid(True)
    plt.show()
